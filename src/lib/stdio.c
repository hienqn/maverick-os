...
static const int MAX_FLOAT_PRECISION = 9;
    ...
    switch (*format) {
      ...
      case 'f': {
        /* Floating-point conversion.
               Note: floating point formatting has been implemented only
               so that floating point numbers of the form <whole>.<decimal>
               can be printed, where <whole> fits in a standard integer and
               <decimal> also fits within a standard integer. */

        // Ensure precision is correct
        if (c.precision < 0)
          c.precision = DEFAULT_FLOAT_PRECISION;
        if (c.precision > MAX_FLOAT_PRECISION)
          c.precision = MAX_FLOAT_PRECISION;
        ...
        int precision = c.precision;
        c.precision = -1;
        format_integer(first < 0 ? -first : first, true, d < 0, &base_d, &c, output, aux);

        // Print the decimal place
        output('.', aux);

        // Print after the decmial
        // Use the correct precision
        c.precision = precision == 0 ? 1 : precision;
        format_integer(rest < 0 ? -rest : rest, false, false, &base_d, &c, output, aux);
      } break;
      ...
      case 'G':
      case 'n':
        /* We don't support printing exponentials,
